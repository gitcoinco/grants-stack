  ...
  describe("core functions", () => {
    ...
    describe("test: updateRoundContract", async () => {
      ...
      it("RoundContract SHOULD emit RoundContractUpdated event after invoking updateRoundContract", async () => {
        await expect(
          roundFactory.updateRoundContract(roundImplementation.address)
        )
      ...
      it("RoundContract SHOULD have round address after invoking updateRoundContract", async () => {
        await roundFactory
          .updateRoundContract(roundImplementation.address)
          .then(async () => {
    ...
    describe("test: create", async () => {
      const applicationsStartTime = Math.round(
        new Date().getTime() / 1000 + 3600
      ); // 1 hour later
      const applicationsEndTime = Math.round(
