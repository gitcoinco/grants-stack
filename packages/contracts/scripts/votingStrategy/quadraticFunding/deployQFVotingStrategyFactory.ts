...
export async function main() {
  ...
  const contractFactory = await ethers.getContractFactory("QuadraticFundingVotingStrategyFactory");
  const contract = await upgrades.deployProxy(contractFactory);

  console.log(`Deploying Upgradable QuadraticFundingVotingStrategyFactory to ${contract.address}`);
