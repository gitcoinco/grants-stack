...
import { handleVotingContractCreated } from "../../../src/votingStrategy/quadraticFundingRelay/factory";
import { VotingContractCreated as VotingContractCreatedEvent } from "../../../generated/QuadraticFundingRelayStrategy/QuadraticFundingRelayStrategyFactory";
...
function createNewVotingContractCreatedEvent(
  votingContractAddress: Address,
  votingImplementation: Address
): VotingContractCreatedEvent {
  const newProgramEvent = changetype<VotingContractCreatedEvent>(
    newMockEvent()
  );

  const votingContractAddressParam = new ethereum.EventParam(
    "votingContractAddress",
    ethereum.Value.fromAddress(votingContractAddress)
  );
  const votingImplementationParam = new ethereum.EventParam(
    "votingImplementation",
    ethereum.Value.fromAddress(votingImplementation)
  );
...
describe("handleVotingContractCreated", () => {
  beforeEach(() => {
    votingContractAddress = Address.fromString(
      "0xA16081F360e3847006dB660bae1c6d1b2e17eC2A"
    );
    votingImplementation = Address.fromString(
  ...
  test("VotingStrategy entity is created when handleVotingContractCreated is called", () => {
    ...
    assert.stringEquals(
      votingStrategy!.strategyName,
      "LINEAR_QUADRATIC_FUNDING"
    );
    assert.stringEquals(
      votingStrategy!.strategyAddress,
      votingImplementation.toHex()
    );
