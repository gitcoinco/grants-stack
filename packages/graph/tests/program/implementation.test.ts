...
import {
  handleRoleGranted,
  handleRoleRevoked,
} from "../../src/program/implementation";
import {
...
function createNewRoleGrantedEvent(
  ...
  const roleParam = new ethereum.EventParam(
    "role",
...
describe("handleRoleGranted", () => {
  beforeEach(() => {
    ...
    programEntity.updatedAt = new BigInt(1);

    programEntity.save();

    newRoleGrantedEvent = createNewRoleGrantedEvent(role, account, program);
  });
