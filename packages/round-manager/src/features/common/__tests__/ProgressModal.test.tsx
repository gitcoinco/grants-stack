...
describe('<ProgressModal />',  () => {
  describe("error status styling", () => {
    it('shows "error" style for the step when the step status is error',  () => {
      renderWrapped(<ProgressModal isOpen setIsOpen={() => {}} steps={steps} />);

      screen.getByTestId(`${errorStep.name}-error-icon`);
    });

    it.each([completedStep, currentStep, upcomingStep])('does not show "error" icon for a step with status $status',  (nonMatchingStep: Step) => {
      renderWrapped(<ProgressModal isOpen setIsOpen={() => {}} steps={steps} />);

      expect(screen.queryByTestId(`${nonMatchingStep.name}-error-icon`)).not.toBeInTheDocument()
    });
  })
  ...
  describe("current status styling", () => {
    ...
    it.each([completedStep, errorStep, upcomingStep])('does not show "current" icon for a step with status $status', (nonMatchingStep: Step) => {
      renderWrapped(<ProgressModal isOpen setIsOpen={() => {}} steps={steps} />);

      expect(screen.queryByTestId(`${nonMatchingStep.name}-current-icon`)).not.toBeInTheDocument()
    })
  })
  ...
  describe("upcoming status styling", () => {
    ...
    it.each([currentStep, errorStep, completedStep])('does not show "upcoming" icon for a step with status $status',  (nonMatchingStep: Step) => {
      renderWrapped(<ProgressModal isOpen setIsOpen={() => {}} steps={steps} />);

      expect(screen.queryByTestId(`${nonMatchingStep.name}-upcoming-icon`)).not.toBeInTheDocument()
    })
  })
  ...
  describe("complete status styling", () => {
    ...
    it.each([currentStep, errorStep, upcomingStep])('does not show "complete" icon for a step with status $status',  (nonMatchingStep: Step) => {
      renderWrapped(<ProgressModal isOpen setIsOpen={() => {}} steps={steps} />);

      expect(screen.queryByTestId(`${nonMatchingStep.name}-complete-icon`)).not.toBeInTheDocument()
    })
  })
});
