...
describe('ViewApplicationPage', () => {
  ...
  ("shows no project %s verification when you have an invalid verifiable credential for it", async (provider: string, projectCredentials: ProjectCredentials) => {
  ...
  ("shows project %s verification when you have a valid verifiable credential for it", async (provider: string, projectCredentials: ProjectCredentials) => {
    ...
    (useListGrantApplicationsQuery as any).mockReturnValue(grantApplicationWithValidVc);
    (useUpdateGrantApplicationMutation as any).mockReturnValue([jest.fn(), { isLoading: false }]);
  ...
  it("shows invalid badge when verifiable credential was not issued by correct IAM server", async () => {
    verifyCredentialMock.mockResolvedValue(true)
    const fakeIssuer = "did:key:z6Mks2YNwbkzDgKLuQs1TS3whP9RdXrGXtVqt5JcCLoQu86W"
    ...
    await waitFor(() => {
      expect(screen.getByTestId(`github-verifiable-credential-unverified`)).toBeInTheDocument()
    })

    expect(screen.queryByTestId(`github-verifiable-credential`)).not.toBeInTheDocument()
  })

  it.each([
    {
      provider: "github",
      projectCredentials: { "github": githubCredentialData } as ProjectCredentials,
    ...
    {
      provider: "twitter",
      projectCredentials: { "twitter": twitterCredentialData } as ProjectCredentials,
      vcProviderString: "ClearTextTwitter#DpoppDev",
      question: "Twitter",
      applicationAccountName: "notDpoppDev",
    }]
  )("shows invalid badge for $provider when $question account name in grant application doesn't match account name in VC provider", async (
    {
      provider,
      projectCredentials,
      vcProviderString,
    ...
    projectCredentials[provider].credentialSubject.provider = vcProviderString

    const grantApplication = {
      application: makeGrantApplicationData({answers: [{questionId: 0, question, answer: applicationAccountName}]}, projectCredentials)
    };
