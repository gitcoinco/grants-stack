...
import { getChainById, stringToBlobUrl, useWallet } from "common";
...
export const TabGroup = () => {
  ...
  const dgRounds = rounds.filter(
    (round) => round?.strategyName === "allov2.DirectGrantsLiteStrategy"
  );
  const dgRoundItems = dgRounds
    ? dgRounds.map((round, index) => {
        if (!round.chainId) return;
