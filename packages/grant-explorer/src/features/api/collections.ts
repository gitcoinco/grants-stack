...
export const createCollection = async ({
  ...
  localStorage.setItem(
    "collections",
    JSON.stringify(Array.from(new Set(parseCollections()).add(id)))
  );
...
export const getCollectionById = async (id: string): Promise<Collection> => {
  ...
  const collection = JSON.parse(
    CryptoJS.AES.decrypt(id, secretKey).toString(CryptoJS.enc.Utf8)
  );
...
export const getAllCollections = async (
  ...
  return parseCollections()
    ...
    .map((collectionKey) =>
      JSON.parse(localStorage.getItem(collectionKey) || "[]")
    )
    .reverse();
};
...
export const addProjectToCollection = async ({
  collection,
  project,
}: AddProjectData) => {
  const collectionData = JSON.parse(localStorage.getItem(collection) || "[]");
  collectionData.projects = Array.from(
    new Set(collectionData.projects).add(project)
  );
