...
export default function NavbarCart(props: { cart: Project[]; roundUrlPath: string }) {
...
function QuickViewSummary (props: {
  roundUrlPath: string;
  cart: Project[]
}) {
  ...
  return (
    <div className="w-72 flex flex-col absolute right-0 rounded bg-white shadow-[0px_0px_10px_0px_rgba(0,0,0,0.1)] py-4 px-4 z-20">
      <p className="flex border-b mb-4 gap-2">
        <CheckIcon className="w-5" />
        Projects in your cart
      </p>

      {/* TODO: Add fixed height + scroll */}
      <div className="mb-4 max-h-[100px] overflow-y-scroll">
      ...
      <Button
        ...
        onClick={() =>
          navigate(`${props.roundUrlPath}/cart`)
        }
      >
        Checkout Cart
      </Button>
