  ...
  const loadProjects = useCallback(
    async (projects: number[]) => {
      setProjectsInCollection(
        (await getProjects(chainId, roundId as string, projects)) as Project[]
      );
    },
    [chainId, roundId]
  );
  ...
  return (
    ...
    >
      <CardHeader>
        <div className="flex flex-1 h-[180px]">
          {projectsInCollection.length > 0 && (
            <CollectionBanner
              height={180}
              images={projectsInCollection
                .map((i) => i.projectMetadata.bannerImg || "")
                .slice(0, 4)}
            />
          )}
        </div>
      ...
      <CardContent className="px-2">
        <CardTitle data-testid="project-title">{collection.title}</CardTitle>
    ...
    <div>
      <div className="flex my-4 justify-between">
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            <li className="inline-flex items-center">
              <span role="button" onClick={backToLists}>
                My Lists
              </span>
            </li>
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="ml-1 font-medium md:ml-2">
                  {collection.title} ({collection.projects.length})
                </span>
              </div>
            </li>
          </ol>
        ...
        <Button
          ...
          onClick={() =>
            copy(
              `${window.location.protocol}//${window.location.host}/#/collection?data=${collection.enc}`
            )
          }
        >
      ...
      <ProjectList
        ...
        isBeforeRoundEndDate={isActiveRound}
        isInCollection={true}
...
const AllCollectionsView = ({ projects, isActiveRound }: Props) => {
  ...
  const loadAllCollections = useCallback(async (round: string) => {
    ...
    setCollections(
      _collections.map((i) => ({
        ...i,
        enc: encodeURIComponent(encodeCollection(JSON.stringify(i))),
      }))
    );
        ...
        <CardsContainer>
          {activeCollection ? (
            <CollectionPreview
              ...
              backToLists={() => setActiveCollection(undefined)}
            />
          ) : (
