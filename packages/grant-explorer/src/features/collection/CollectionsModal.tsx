...
const CollectionItem = (props: {
  ...
  id: string;
  projects: number[];
  addToCollectionHandler: (id: string) => Promise<void>;
}) => {
  ...
  return (
    <div
      className="flex flex-1 justify-between mb-4 text-base p-4 bg-grey-150 items-center"
      aria-role="button"
      onClick={handlerAdd}
    >
      ...
      <div className="h-[56px] w-[112px] bg-white rounded">
        {projectsImages.length > 0 && (
          <CollectionBanner height={56} images={projectsImages} />
        )}
      </div>
    </div>
  );
...
export const CollectionsModal = ({
  ...
  return (
    <Modal title="Choose a list" open={open} closeModal={closeModal}>
      <div className="mt-4 z-30">
        ...
        {myCollections.length === 0 && !loadingCollections && (
          <div className="flex flex-1 items-center justify-center my-4 text-sm">
        ...
        {!loadingCollections &&
          myCollections.map((collectionItem) => (
            <CollectionItem
              ...
              addToCollectionHandler={addToCollection}
              projects={collectionItem.projects}
            />
          ))}
      </div>
...
export const NewCollectionsModal = ({ open, closeModal, project }: Props) => {
  ...
  return (
    <Modal title="Name your list" open={open} closeModal={closeModal}>
      <div className="mt-4 z-30">
