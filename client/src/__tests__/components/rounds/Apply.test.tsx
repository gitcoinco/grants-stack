...
describe("<Apply />", () => {
  describe("with a valid round submit an application", () => {
    ...
    beforeEach(() => {
      ...
      store.dispatch({ type: "ROUND_APPLICATION_LOADED", roundAddress: "0x1234", projectId: "1" });
    });

    describe("application status", () => {
      it("should be sent", async () => {
        expect(store.getState().roundApplication["0x1234"].status).toBe(Status.Sent);
      });
    });

    describe("should redirect to project details page", () => {
      it("should pass project id to route", async () => {
        expect(store.getState().roundApplication["0x1234"].projectsIDs[0]).toBe("1");
      });
    });
  });
});
